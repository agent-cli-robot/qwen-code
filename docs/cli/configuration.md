...
Agent CLI offers several ways to configure its behavior, including environment variables, command-line arguments, and settings files. This document outlines the different configuration methods and available settings.
...
Agent CLI uses JSON settings files for persistent configuration. There are four locations for these files:

- **System defaults file:**
  - **Location:** `/etc/agent-cli/system-defaults.json` (Linux), `C:\ProgramData\agent-cli\system-defaults.json` (Windows) or `/Library/Application Support/AgentCLI/system-defaults.json` (macOS). The path can be overridden using the `AGENT_CLI_SYSTEM_DEFAULTS_PATH` environment variable.
...
- **User settings file:**
  - **Location:** `~/.agent/settings.json` (where `~` is your home directory).
  - **Scope:** Applies to all Agent CLI sessions for the current user.
- **Project settings file:**
  - **Location:** `.agent/settings.json` within your project's root directory.
  - **Scope:** Applies only when running Agent CLI from that specific project. Project settings override user settings.

- **System settings file:**
  - **Location:** `/etc/agent-cli/settings.json` (Linux), `C:\ProgramData\agent-cli\settings.json` (Windows) or `/Library/Application Support/AgentCLI/settings.json` (macOS). The path can be overridden using the `AGENT_CLI_SYSTEM_SETTINGS_PATH` environment variable.
  - **Scope:** Applies to all Agent CLI sessions on the system, for all users. System settings override user and project settings. May be useful for system administrators at enterprises to have controls over users' Agent CLI setups.

**Note on environment variables in settings:** String values within your `settings.json` files can reference environment variables using either `$VAR_NAME` or `${VAR_NAME}` syntax. These variables will be automatically resolved when the settings are loaded. For example, if you have an environment variable `MY_API_TOKEN`, you could use it in `settings.json` like this: `"apiKey": "$MY_API_TOKEN"`.

### The `.agent` directory in your project

In addition to a project settings file, a project's `.agent` directory can contain other project-specific files related to Agent CLI's operation, such as:

- [Custom sandbox profiles](#sandboxing) (e.g., `.agent/sandbox-macos-custom.sb`, `.agent/sandbox.Dockerfile`).

### Available settings in `settings.json`:

- **`contextFileName`** (string or array of strings):\n - **Description:** Specifies the filename for context files (e.g., `AGENTS.md`). Can be a single filename or a list of accepted filenames.\n - **Default:** `AGENTS.md`\n - **Example:** `"contextFileName": "AGENTS.md"`

- **`bugCommand`** (object):
  - **Description:** Overrides the default URL for the `/bug` command.
  - **Default:** `"urlTemplate": "https://github.com/agent-cli/agent-cli/issues/new?template=bug_report.yml&title={title}&info={info}"`
  - **Properties:**
...
1.  **Use `.agentignore`:** Create a `.agentignore` file in your project root to exclude directories that contain a large number of files that you don't need to reference (e.g., build artifacts, logs, `node_modules`). Reducing the total number of files crawled is the most effective way to improve performance.
...
- **`theme`** (string):
  - **Description:** Sets the visual [theme](./themes.md) for Agent CLI.
  - **Default:** `"Default"`
...
- **`sandbox`** (boolean or string):
  - **Description:** Controls whether and how to use sandboxing for tool execution. If set to `true`, Agent CLI uses a pre-built `agent-cli-sandbox` Docker image. For more information, see [Sandboxing](#sandboxing).
  - **Default:** `false`
...
- **`mcpServers`** (object):
  - **Description:** Configures connections to one or more Model-Context Protocol (MCP) servers for discovering and using custom tools. Agent CLI attempts to connect to each configured MCP server to discover available tools. If multiple MCP servers expose a tool with the same name, the tool names will be prefixed with the server alias you defined in the configuration (e.g., `serverAlias__actualToolName`) to avoid conflicts. Note that the system might strip certain schema properties from MCP tool definitions for compatibility. At least one of `command`, `url`, or `httpUrl` must be provided. If multiple are specified, the order of precedence is `httpUrl`, then `url`, then `command`.
  - **Default:** Empty
  - **Properties:**
...
- **`telemetry`** (object)
  - **Description:** Configures logging and metrics collection for Agent CLI. For more information, see [Telemetry](../telemetry.md).
...
- **`excludedProjectEnvVars`** (array of strings):
  - **Description:** Specifies environment variables that should be excluded from being loaded from project `.env` files. This prevents project-specific environment variables (like `DEBUG=true`) from interfering with the CLI behavior. Variables from `.agent/.env` files are never excluded.
...
- **`loadMemoryFromIncludeDirectories`** (boolean):
  - **Description:** Controls the behavior of the `/memory refresh` command. If set to `true`, `AGENTS.md` files should be loaded from all directories that are added. If set to `false`, `AGENTS.md` should only be loaded from the current directory.
  - **Default:** `false`
  - **Example:**
    ```json
...
- **Location:** `~/.agent/tmp/<project_hash>/shell_history`
...
**Environment Variable Exclusion:** Some environment variables (like `DEBUG` and `DEBUG_MODE`) are automatically excluded from project `.env` files by default to prevent interference with the CLI behavior. Variables from `.agent/.env` files are never excluded. You can customize this behavior using the `excludedProjectEnvVars` setting in your `settings.json` file.
...
- **`SEATBELT_PROFILE`** (macOS specific):
  ...
  - `<profile_name>`: Uses a custom profile. To define a custom profile, create a file named `sandbox-macos-<profile_name>.sb` in your project's `.agent/` directory (e.g., `my-project/.agent/sandbox-macos-custom.sb`).
- **`DEBUG` or `DEBUG_MODE`** (often used by underlying libraries or the CLI itself):
  - Set to `true` or `1` to enable verbose debug logging, which can be helpful for troubleshooting.
  - **Note:** These variables are automatically excluded from project `.env` files by default to prevent interference with the CLI behavior. Use `.agent/.env` files if you need to set these for Agent CLI specifically.
- **`NO_COLOR`**:
...
- **`--model <model_name>`** (**`-m <model_name>`**):
  ...
  - Example: `npm start -- --model <model-name>`
- **`--prompt <your_prompt>`** (**`-p <your_prompt>`**):
  - Used to pass a prompt directly to the command. This invokes Agent CLI in a non-interactive mode.
- **`--prompt-interactive <your_prompt>`** (**`-i <your_prompt>`**):
  - Starts an interactive session with the provided prompt as the initial input.
  - The prompt is processed within the interactive session, not before it.
  - Cannot be used when piping input from stdin.
  - Example: `agent -i "explain this code"`
- **`--sandbox`** (**`-s`**):
...
- **`--approval-mode <mode>`**:
  ...
  - Example: `agent --approval-mode auto-edit`
- **`--allowed-tools <tool1,tool2,...>`**:
  - A comma-separated list of tool names that will bypass the confirmation dialog.
  - Example: `agent --allowed-tools "ShellTool(git status)"`
- **`--telemetry`**:
...
- **`--extensions <extension_name ...>`** (**`-e <extension_name ...>`**):
  ...
  - Use the special term `agent -e none` to disable all extensions.
  - Example: `agent -e my-extension -e my-other-extension`
- **`--list-extensions`** (**`-l`**):
...
- **Hierarchical Loading and Precedence:** The CLI implements a sophisticated hierarchical memory system by loading context files (e.g., `AGENTS.md`) from several locations. Content from files lower in this list (more specific) typically overrides or supplements content from files higher up (more general). The exact concatenation order and final context can be inspected using the `/memory show` command. The typical loading order is:
  1.  **Global Context File:**
      - Location: `~/.agent/<contextFileName>` (e.g., `~/.agent/AGENTS.md` in your user home directory).
...
By understanding and utilizing these configuration layers and the hierarchical nature of context files, you can effectively manage the AI's memory and tailor Agent CLI's responses to your specific needs and projects.

## Sandboxing

Agent CLI can execute potentially unsafe operations (like shell commands and file modifications) within a sandboxed environment to protect your system.
...
By default, it uses a pre-built `agent-cli-sandbox` Docker image.

For project-specific sandboxing needs, you can create a custom Dockerfile at `.agent/sandbox.Dockerfile` in your project's root directory. This Dockerfile can be based on the base sandbox image:

```dockerfile
FROM agent-cli-sandbox
...
When `.agent/sandbox.Dockerfile` exists, you can use `BUILD_SANDBOX` environment variable when running Agent CLI to automatically build the custom sandbox image:

```bash
BUILD_SANDBOX=1 agent -s
```

## Usage Statistics

To help us improve Agent CLI, we collect anonymized usage statistics. This data helps us understand how the CLI is used, identify common issues, and prioritize new features.
...
- **`enableWelcomeBack`** (boolean):
  ...
  - **Details:** When enabled, Agent CLI will automatically detect if you're returning to a project with a previously generated project summary (`.agent/PROJECT_SUMMARY.md`) and show a dialog allowing you to continue your previous conversation or start fresh. This feature integrates with the `/chat summary` command and quit confirmation dialog. See the [Welcome Back documentation](./welcome-back.md) for more details.
