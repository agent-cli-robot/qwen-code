...
import { createTmpDir, cleanupTmpDir } from '@agent-cli/agent-cli-test-utils';
...
describe('FileSearch', () => {
  ...
  it('should use .agentignore rules', async () => {
    ...
    expect(results).toEqual(['src/', '.agentignore', 'src/not-ignored.js']);
  });

  it('should combine .gitignore and .agentignore rules', async () => {
    tmpDir = await createTmpDir({
      '.agentignore': 'build/',
      '.gitignore': 'dist/',
      dist: ['ignored-by-git.js'],
      build: ['ignored-by-agent.js'],
      src: ['not-ignored.js'],
    });
