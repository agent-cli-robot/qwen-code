...
describe('FileDiscoveryService', () => {
  ...
  describe('initialization', () => {
    ...
    it('should load .agentignore patterns even when not in a git repo', async () => {
      await createTestFile('.agentignore', 'secrets.txt');
      const service = new FileDiscoveryService(projectRoot);

      expect(service.shouldAgentIgnoreFile('secrets.txt')).toBe(true);
      expect(service.shouldAgentIgnoreFile('src/index.js')).toBe(false);
    });
  });
  ...
  describe('filterFiles', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/\n.git/\ndist');
      await createTestFile('.agentignore', 'logs/');
    });
    ...
    it('should not filter files when respectGitIgnore is false', () => {
      ...
      const filtered = service.filterFiles(files, {
        ...
        respectAgentIgnore: true, // still respect this one
      });
    ...
    it('should not filter files when respectAgentIgnore is false', () => {
      const files = [
        'src/index.ts',
        'node_modules/package/index.js',
        'logs/latest.log',
  ...
  describe('shouldGitIgnoreFile & shouldAgentIgnoreFile', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/');
    ...
    it('should return true for gemini-ignored files', () => {
      ...
      expect(
        service.shouldAgentIgnoreFile(path.join(projectRoot, 'debug.log')),
      ).toBe(true);
    });
    ...
    it('should return false for non-gemini-ignored files', () => {
      ...
      expect(
        service.shouldAgentIgnoreFile(path.join(projectRoot, 'src/index.ts')),
      ).toBe(false);
    });
  });
